...
async fn test_setup_pki() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ttl = "8760h";
    let (cert, role_name) =
        merka_vault::vault::setup_pki(&vault_url, "root", domain, ttl, false, None, None).await?;
...
async fn test_setup_pki_same_vault_intermediate() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (cert_chain, role_name) =
        merka_vault::vault::setup_pki(&vault_url, "root", domain, ttl, true, None, None).await?;
...
async fn test_setup_pki_secondary_vault_intermediate() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let init_res = merka_vault::vault::init_vault(&root_url, 5, 3).await?;
    assert!(!init_res.root_token.is_empty());
    merka_vault::vault::unseal_vault(&root_url, &init_res.keys).await?;
    let root_token = init_res.root_token;

    // Initialize and unseal the intermediate Vault.
    let int_init_res = merka_vault::vault::init_vault(&int_url, 5, 3).await?;
    assert!(!int_init_res.root_token.is_empty());
    merka_vault::vault::unseal_vault(&int_url, &int_init_res.keys).await?;
    let int_root_token = int_init_res.root_token; // Use this token for intermediate Vault operations

    let domain = "example.com";
    let ttl = "8760h";
    let (cert_chain, role_name) = merka_vault::vault::setup_pki_intermediate(
        &root_url,
        &root_token,
        &int_url,
        &int_root_token,
...
async fn test_vault_init_and_unseal() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(merka_vault::vault::init_vault(&vault_url, 1, 1)
        .await
        .is_err());
...
async fn test_full_vault_setup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ttl = "8760h";
    let (cert, role_name) =
        merka_vault::vault::setup_pki(&vault_url, &root_token, domain, ttl, false, None, None)
            .await?;
    assert!(cert.contains("BEGIN CERTIFICATE"));
    println!(
        "PKI setup complete: role '{}' for domain {}, CA cert length {}",
...
async fn test_issue_cert_and_verify_tls() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialize and unseal the root Vault.
    let root_init = merka_vault::vault::init_vault(&root_url, 5, 3).await?;
    assert!(!root_init.root_token.is_empty());
    merka_vault::vault::unseal_vault(&root_url, &root_init.keys).await?;
    let root_token = root_init.root_token;

    // Initialize and unseal the intermediate Vault.
    let int_init = merka_vault::vault::init_vault(&int_url, 5, 3).await?;
    assert!(!int_init.root_token.is_empty());
    merka_vault::vault::unseal_vault(&int_url, &int_init.keys).await?;
    ...
    let domain = "example.com";
    let ttl = "24h";
    let (pki_chain, role_name) = merka_vault::vault::setup_pki_intermediate(
        &root_url,
        &root_token,
        &int_url,
        &int_token,
    ...
    let (issued_cert, issued_key) = merka_vault::vault::issue_certificate(
        &int_url,
        &int_token,
        &role_name,
        domain,
